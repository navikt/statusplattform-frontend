...
const Incidents: React.FC<{ allOpsMessages: OpsMessageI[] }> = ({
    allOpsMessages,
}) => {
    ...
    if (allOpsMessages.length == 0) {
        return (
            <IncidentsPage>
                <BodyShort>Du har ingen driftsmeldinger.</BodyShort>
            </IncidentsPage>
        )
    }
...
export const IncidentCard: React.FC<{
    ...
    return (
        <PanelCustomized border className="incident-row">
            ...
            <BodyShort spacing className="time-frame" size="small">
                {timeframe}
            </BodyShort>
            ...
            <Button
                variant="tertiary"
                onClick={() =>
                    router.push(`${RouterOpsMeldinger.PATH}/${opsId}`)
                }
            >
                Se mer...
            </Button>
