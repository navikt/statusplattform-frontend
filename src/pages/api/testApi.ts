import { NextApiRequest, NextApiResponse } from "next";





const createRequest = (path,method ,headers)  => new Request(path, {
    method: method,
    headers: headers,
  });

export default async (req: NextApiRequest, res: NextApiResponse) => {


   let path = EndPathGetLoginInfo ;// EndPathGetLoginInfo();
   //let backEndRequest = createRequest(path, "GET", req.headers);

    const myHeaders = new Headers();
    myHeaders.append("authorization","Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6ImpTMVhvMU9XRGpfNTJ2YndHTmd2UU8yVnpNYyJ9..ANvw02fedR9ITFxsAdaJg__lIg6ihuf3MU9dqutp5boZ8hSfNjoSswVOc96pbSuvxL28R1ErhX2R4ssFqVvvk2O9-pt2pt4R59WS03rWXty88Er526x6TtrWypU0wBxv3H3ExE4eqaY9boyrg9UiRYCnzb2e92RG81_afn6rd6Fq16LDtm6fHZ4krt-RXj_oYnKMf7VtTScPXG-axA6Lii51a7oPvTBa9v-6P-MdeSIWH4JErUtXR2YVF0eIfNSO3xhg4tZtApnA6VGNzISyO7ayOKGlo2p9axAlTYsi9i077FpSgYCmSaN6P9fK2S7pVOodVgo7FJXld82bj5UWgg")

   //let backEndRequest = createRequest(path, "GET", myHeaders);
   let backEndRequest = new Request(EndPathGetLoginInfo(), {
    method: "GET",
    headers: myHeaders,
  });
  let result = await fetch(backEndRequest);
    if (result.ok) {
        console.log("SUCCSESFULLY READ USERDATA")
        console.log(result.clone().json())
        res.send(result.json());
    }
    else {
    res.send("Cant read userdate")
    }
    

};




  
// Login
const EndPathGetLoginInfo = () => {
    let devpath = "https://digitalstatus.ekstern.dev.nav.no";
    let localPath = "http://localhost:3005";
    return localPath + "/authenticate/NavUser";
}
// ---


 class ResponseError extends Error {
    public constructor (message: string, public response: Response) {
        super(message)
    }
}


//FOR TESTING LOKALT KAN DENNE HEADEREN LEGGES PÅ ISTEDEN FOR FRA REQUESTEN FRA FRONTEN, SOM IKKE VILL INNEHOLDE authorization VED LOKAL KJØRING.  
